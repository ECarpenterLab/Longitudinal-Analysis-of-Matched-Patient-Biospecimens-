correct_reads <- function(data_dir, rho = NULL , extra = NULL, cap = FALSE){
  ...
  # importing dataset and make soup channel with the raw and filtered matrix files
  #adjusted for .h5 file formats rather than filtered or raw matrix feature directories
  raw_mtx      <- Read10X_h5(paste0(data_dir, "/raw_feature_bc_matrix.h5"))
  filt_mtx     <- Read10X_h5(paste0(data_dir, "/filtered_feature_bc_matrix.h5"))
  sc  <- SoupChannel(raw_mtx, filt_mtx) #make the soup channel that load10X would have internally when using filtered_feature_matrix directories
  
  
  # Creating seurat clusters
  seurat_obj <- CreateSeuratObject(filt_mtx)

...
pca_plot <- function(dds, groups){
  ...
  pca_plot <- biplot(pca, showLoadings = F, boxedLoadingsNames = T,
                     ...
                     labSize = 4, pointSize = 5, drawConnectors = T,
...
HallMarks_GSEA_enrich <- function(dge){
  ...
  gene_rank <- dge$log2FoldChange %>% `names<-`(rownames(dge)) %>% sort()
  gene_rank <- gene_rank[!is.na(gene_rank)]
  H_gsea <- fgsea(pathways = hallmarks_pathways,
                  stats = gene_rank)
  
  return(H_gsea)
}
####aylin there are no plots for gsea yet!
